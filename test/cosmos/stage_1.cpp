...
namespace abstractions::bitcoin::cosmos::test {
    ...
    one::one(string secret_wif, 
        ...
        if (SecretWIF.write() != secret_wif) throw failure{};
        if (PubkeyHex.write() != pubkey_hex) throw failure{};
        if (AddressBase58.write() != addr_58) throw failure{};
    }
    
    vector<one> one::cases() {
