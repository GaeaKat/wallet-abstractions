...
namespace abstractions
{
    ...
    namespace colored
    {
        ...
        struct colored_value {
            ...
            colored_value(ℕ v, color_type c) : Value(c == null_color<color_type> ? 0 : v), Color(v == 0 ? null_color<color_type> : c) {}
        };
        ...
        struct colored_output {
            ...
            colored_output() : Index(ℵ0), Transaction() {}
            colored_output(ℕ i, transaction t) : Index(i), Transaction(t) {}
        };
        ...
        template<typename outpoint, typename transaction, typename output, typename color_type>
        colored_value<color_type> value(transaction, ℕ index, blockchain<outpoint, transaction, output, color_type>& b);
        ...
        template <typename transaction>
        inline ℕ padding(transaction t) {
        ...
        template<typename outpoint, typename transaction, typename output, typename color_type>
        colored_value<color_type> value(transaction t, ℕ index, blockchain<outpoint, transaction, output, color_type>& b) {
